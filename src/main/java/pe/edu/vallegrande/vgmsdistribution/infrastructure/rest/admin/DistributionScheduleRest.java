...
public class DistributionScheduleRest {
    ...
    // New endpoint to get all enriched distribution schedules
    @GetMapping("/enriched")
    public Mono<ResponseDto<List<EnrichedDistributionScheduleResponse>>> getAllEnriched() {
        return service.getAllEnriched()
                .collectList()
                .map(result -> new ResponseDto<>(true, result));
    }
    ...
    // New endpoint to get all active enriched distribution schedules
    @GetMapping("/active/enriched")
    public Mono<ResponseDto<List<EnrichedDistributionScheduleResponse>>> getAllActiveEnriched() {
        return service.getAllActiveEnriched()
                .collectList()
                .map(result -> new ResponseDto<>(true, result));
    }
    ...
    // New endpoint to get all inactive enriched distribution schedules
    @GetMapping("/inactive/enriched")
    public Mono<ResponseDto<List<EnrichedDistributionScheduleResponse>>> getAllInactiveEnriched() {
        return service.getAllInactiveEnriched()
                .collectList()
                .map(result -> new ResponseDto<>(true, result));
    }
    ...
    // New endpoint to get enriched distribution schedule by ID
    @GetMapping("/{id}/enriched")
    public Mono<ResponseDto<EnrichedDistributionScheduleResponse>> getEnrichedById(@PathVariable String id) {
        return service.getEnrichedById(id)
                .map(result -> new ResponseDto<>(true, result))
                .onErrorResume(e -> Mono.just(
                        new ResponseDto<>(false,
                                new ErrorMessage(HttpStatus.NOT_FOUND.value(),
    ...
    // New endpoint to create a schedule and return enriched response
    @PostMapping("/enriched")
    @ResponseStatus(HttpStatus.CREATED)
    public Mono<ResponseDto<EnrichedDistributionScheduleResponse>> createAndEnrich(@RequestBody DistributionScheduleCreateRequest request) {
        return service.saveAndEnrich(request)
                .map(saved -> new ResponseDto<>(true, saved))
                .onErrorResume(e -> Mono.just(
                        new ResponseDto<>(false,
                                new ErrorMessage(HttpStatus.BAD_REQUEST.value(),
